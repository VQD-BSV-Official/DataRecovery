...
Cách dùng:
  python jpeg_scanner.py -i card.img                   # scan + log bên cạnh file input
  python jpeg_scanner.py -i card.img -o ./recovered    # scan + log + extract ảnh ra folder
"""
...
# ── Hằng số ───────────────────────────────────────────────────────────────────
SECTOR     = 512
CHUNK_SIZE = 256 * 1024 * 1024     # 256 MB / lần đọc
...
def short_cam(make: str, model: str) -> str:
    ...
    if model:
        for tok in model.split():
            t = tok.lower().rstrip('.,')
            if t not in noise and t != make0.lower():
                parts.append(tok)
...
def process_chunk(buf: bytes, base_off: int, min_bytes: int,
    ...
    if carry is not None:
        ...
        lo = np.searchsorted(eois_ext, 0,   'left')
        ...
        if lo < hi:
            ...
            # EOI chưa thấy → tiếp tục carry, cộng dồn size
...
# ── Extract ảnh ra file ───────────────────────────────────────────────────────
def extract_images(filepath: str, results: list, out_dir: str):
    """
    Đọc lại file input, cắt từng ảnh theo abs_offset + size_bytes,
    ghi ra out_dir/IMG_XXXX.JPG
    """
    if not results:
        return

    # print(f"\n{B}{'─'*68}{R}")
    # print(f"  {C}💾 Extract {len(results)} ảnh → {out_dir}{R}")
    # print(f"{B}{'─'*68}{R}")
    ...
    with open(filepath, 'rb') as fd:
        for idx, r in enumerate(results, 1):
            ...
            fd.seek(r['abs_offset'])
            data = fd.read(r['size_bytes'])

            with open(out_path, 'wb') as wf:
                wf.write(data)

    #         clear_bar()
    #         print(f"  {G}[#{idx:04d}]{R}  {out_name}  "
    #               f"{Y}{fmt_sz(r['size_bytes'])}{R}  "
    #               f"{D}{r['make']} {r['model']}{R}")

    # print(f"\n  {G}✅ Đã lưu {len(results)} ảnh vào:{R} {out_dir}\n")
...
def main():
    ap = argparse.ArgumentParser(
        ...
        epilog="""
Ví dụ:
  # Chỉ scan + lưu log (bên cạnh file input)
  python jpeg_scanner.py -i card.img

  # Scan + lưu log + extract ảnh ra folder
  python jpeg_scanner.py -i card.img -o ./recovered

  # Tùy chọn thêm
  python jpeg_scanner.py -i 16GB.dsk -o ./out --min-size 1.0 -v
  sudo python jpeg_scanner.py -i /dev/sdc -o /mnt/results
        """
    )
    ...
    args_def = [
        ...
        (('-o', '--output'), {
            'default': None,
            'metavar': 'DIR',
            'help': 'Thư mục để extract ảnh JPEG ra (IMG_0001.JPG, ...). '
                    'Tự tạo nếu chưa có. Bỏ qua = chỉ scan + log.'
        }),
        (('--min-size',), {
            'type': float,
            'default': MIN_IMG_MB,
    ...
    # Log luôn lưu bên cạnh file input (lấy tên file input, đổi đuôi .log)
    input_stem = os.path.splitext(os.path.basename(args.input))[0]
    input_dir  = os.path.dirname(os.path.abspath(args.input))
    log_path   = os.path.join(input_dir, input_stem + ".log")

    # Nếu có -o: tạo folder để extract ảnh
    out_dir = None
...
{B}{C}  ╔══════════════════════════════════════╗
  ...
  {D}numpy · sector-aligned · uint16 EOI scan{R}""")
    ...
    results = scan(args.input, log_path, args.min_size, args.verbose)

    if not args.no_table:
        print_table(results)

    # Extract ảnh nếu có -o
    if out_dir is not None:
        extract_images(args.input, results, out_dir)
